...
- `持仓定投` 页只保留 `定投计划` 明细表，标题右侧展示当前持有总额和定投中总额；表内 `评级` 列与主表使用同一套梯队评分结果。
...
申购状态纳入每天 3 次刷新范围：生成器每次都会从东方财富移动接口重新读取 `SGZT`，并同步更新规范化后的 `申购状态`、接口原文 `subscription_status_raw`、代销/平台限额 `daily_limit`、规模和收益字段。直销限额来自 `direct_limits.json` 或脚本回退值，不等同于每次自动重新核验基金公司公告；只有该文件被更新或脚本数据变化时，直销限额才会产生自动对比提示。

页面会把本次刷新和上一版 `nasdaq_fund_snapshot.json` 做执行信息对比，并把结果写入 `execution_alerts`。最近 72 小时内的变化会保留在主表和移动端卡片：限额上调显示绿色 `++` 和“代销上调/直销上调”，限额下调显示红色 `--` 和“代销下调/直销下调”；`暂停申购 -> 允许申购` 显示绿色“恢复申购”，`允许申购 -> 暂停申购` 显示红色“暂停申购”。这些提示只是交易执行提醒，不参与梯队评级。
...
- 快照必须包含 `execution_monitor` 和 `execution_alerts`，且每只基金的 `execution_alert` 要和顶层提示一致。
...
- `execution_alerts`：申购状态、代销限额、直销限额的变化提醒，用于追溯恢复申购、暂停申购或限额上调/下调。页面和 JSON 只保留短窗口提示，SQLite 表保留历史事件，不随当前页面提示清空。
...
- `v_recent_execution_alerts`：最近申购状态和限额变化。
...
- 持有合计：`560元`。
- 定投中合计：`320元 / 期`，其中汇添富 `018966=100元 / 期`，南方 `016452=50元 / 期`，建信 `539001=50元 / 期`，万家 `019441=50元 / 期`。
- 暂停定投合计：`0元 / 期`；旧截图剩余未展开暂停项不写入明细。
...
- 定投金额是“下一次计划现金流”，不是“当前已持仓金额”。不要把 `320元 / 期` 自动加进 `holding_plan.holding_total` 或 `portfolio_records.holding_total`；只有确认扣款/成交后，才通过用户截图、手动修改常量或未来 `transactions` 流水更新持仓。
...
- 点击定投计划表里的 `评级`、`状态`、`金额`、`当前持有` 表头：只重排这张定投计划表，不影响主表排序；手机端表头会卡片化隐藏，因此用表上方的四个排序按钮执行同一套排序。
- 页面会即时更新主表的 `持仓 / 定投` 列、定投计划表和标题总额。
...
- 执行信息变化提示的颜色语义固定：绿色表示平台恢复可买或限额变大，红色表示平台暂停申购或限额变小，黄色表示用户手动/计划层面的 `暂停定投`。不要把黄色暂停定投和红色暂停申购混用。
