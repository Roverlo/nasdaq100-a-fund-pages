...
- 当前定投中金额按用户 2026-06-22 23:22/23:23 支付宝“我的定投”截图和 2026-06-23 口述更新：汇添富 `018966=100`，南方 `016452=50`，建信 `539001=50`，万家 `019441=50`，华安 `040046=10`，广发 `270042=10`，摩根 `019172=10`，招商 `019547=10`，大成 `000834=10`，宝盈 `019736=10`，华泰柏瑞 `019524=10`，合计 `320/期`。
- 当前暂停定投金额：暂无已确认明细，合计 `0/期`。旧截图显示“已暂停(2)”但未展开完整明细，其中建信 `539001` 已由用户 2026-06-23 确认为重新启动 `50/期`，剩余未展开暂停项不能臆造。
- 如果用户口述或新截图更新了旧截图里的定投状态，不只改 `AUTO_INVEST_AMOUNTS` / `PAUSED_AUTO_INVEST_AMOUNTS`；还要同步更新 `AUTO_INVEST_SCREENSHOT_SUMMARY` 的 `source`、`confirmed_active_codes`、`confirmed_paused_codes` 和 `note`，让来源摘要反映“新口径覆盖旧截图”。
