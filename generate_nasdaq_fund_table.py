...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .chart-series-3 {{ stroke: var(--warn); }}
    .chart-point {{
      stroke: var(--panel);
      stroke-width: 2;
    }}
    ...
    .legend-dot-3 {{ background: var(--warn); }}
    .allocation-list {{
      display: grid;
      gap: 10px;
      padding: 14px;
...
{mobile_cards}
  ...
  <script>
    (function () {{
      ...
      function legacyAmountIsPositive(value) {{
      ...
      function statusFromSavedItem(item) {{
        ...
        if (legacyAmountIsPositive(item?.active)) return "定投中";
        if (legacyAmountIsPositive(item?.paused)) return "暂停定投";
        return "候选";
      }}
